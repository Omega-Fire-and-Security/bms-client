...
public class PurchaseOrderManager extends BusinessObjectManager
{
    ...
    public static void emailPurchaseOrder(PurchaseOrder purchaseOrder, Callback callback)
    {
        ...
        //upload purchaseOrder PDF to server
        ...
        final TextField txt_purchaseorder_id = new TextField();
        txt_purchaseorder_id.setMinWidth(200);
        txt_purchaseorder_id.setMaxWidth(Double.MAX_VALUE);
        txt_purchaseorder_id.setPromptText("Type in a message");
        txt_purchaseorder_id.setEditable(false);
        txt_purchaseorder_id.setText(String.valueOf(purchaseOrder.get_id()));
        HBox hbox_purchaseorder_id = CustomTableViewControls.getLabelledNode("Purchase Order ID: ", 200, txt_purchaseorder_id);
        ...
        submit = CustomTableViewControls.getSpacedButton("Send", event ->
        {
            ...
            {
                IO.log(PurchaseOrderManager.class.getName(), IO.TAG_ERROR, e.getMessage());
            }
        });
        ...
        vbox.getChildren().add(hbox_purchaseorder_id);
    ...
    /*public void requestPOApproval(PurchaseOrder po, Callback callback)
    {
        ...
        //gather list of Employees with enough clearance to approve purchaseOrders
        ...
        submit = CustomTableViewControls.getSpacedButton("Send", event ->
        {
            ...
            try
            {
                ...
                HttpURLConnection connection = RemoteComms.postData("/api/purchaseorder/request_approval", params, headers);
                if(connection!=null)
                {
                    if(connection.getResponseCode()==HttpURLConnection.HTTP_OK)
                    {
                        //TODO: CC self
                        IO.logAndAlert("Success", "Successfully requested purchase order #"+po.getNumber()+" approval from ["+cbx_destination.getValue()+"]!", IO.TAG_INFO);
                        if(callback!=null)
                            callback.call(null);
                    }else{
    ...
    public static void uploadPurchaseOrderPDF(PurchaseOrder purchaseOrder)
    {
        if(purchaseOrder==null)
        {
        ...
        if(smgr.getActive()!=null)
        {
            if(!smgr.getActive().isExpired())
            {
                try
                {
                    ...
                    if(path!=null)
                    {
                        ...
                        if (f != null)
                        {
                            if (f.exists())
                            {
                                ...
                                IO.log(PurchaseOrderManager.class.getName(), IO.TAG_INFO, "\n uploaded purchase order [#" + purchaseOrder.get_id()
                    ...
                    } else IO.log(PurchaseOrderManager.class.getName(), "Could not get valid path for created purchase order pdf.", IO.TAG_ERROR);
                } catch (IOException e)
                {
                    IO.log(PurchaseOrderManager.class.getName(), e.getMessage(), IO.TAG_ERROR);
                }
